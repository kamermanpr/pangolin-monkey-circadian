...
if(!dir.exists('figures')) {
    dir.create('figures')
    dir.create('figures/P01-periodicity')
} else {
    if(!dir.exists('figures/P01-periodicity')) {
        dir.create('figures/P01-periodicity')
    }
}
...
knitr::opts_chunk$set(warning = FALSE,
                      ...
                      fig.retina = 2,
                      fig.align = 'center',
                      fig.path = 'figures/P01-periodicity/')
```

----

This script includes analyses on whether there is evidence of circadian variation in body temperature in pangolins. The analysis is for one animal (**P01**) and includes chronograms, actograms, periodgrams, and autocorrelation.
...
data <- read_rds('data-cleaned/pangolin/P01-cleaned.rds')
...
As a preliminary assessment of whether there may be circadian osscilation in body temperature, we plotted chronograms of raw and smoothed _(rolling 60-minute average)_ body temperature data for each month in each calendar year. 
    ...
    mutate(data_binned = map(.x = data,
                             ~ .x %>% 
                                 ...
                                 mutate(day = factor(day,
                                                     ...
                                                     ordered = TRUE)))) %>% 
    # Plot data
    mutate(actograms = pmap(.l = list(data_binned, month, year,
                                      animal, photo_period),
                           ~ ggplot() +
                               geom_rect(data = ..5, 
                                         aes(xmax = sunset, 
...
**Please note that the sequence of months is March 2016 through to March 2017.**
...
**Please note that the sequence of months is March 2016 (light grey) through to March 2017 (dark grey).**
...
data_minmax %>% 
    ...
    geom_vline(xintercept = 0,
               colour = '#000000',
               linetype = 2) +
    labs(title = str_glue('{animal}: Density plot of the time difference between daily\nminimum and maximum body temprature'),
         caption = str_glue('Removed days with multiple maxima or minima:\n {multi_minmax}'),
